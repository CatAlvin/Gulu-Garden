...
Version 2.0.0 — 完美品质系统基础版

当前版本已经完成基础种菜闭环、简单任务系统、图鉴基础版、多作物基础版、土地解锁基础版、背包界面基础版、正式商店面板基础版、任务栏 UI 基础版、中文提示统一整理版，并完成一轮轻量 UI 结构整理：
...
* 支持任务栏 UI 基础版
* 支持中文化玩家反馈提示
* 新增 `src/ui/panel.py` 通用面板绘制 helper
* 新增通用圆角矩形绘制函数
* 新增通用圆角面板绘制函数
* HUD、任务栏、背包、商店、图鉴面板外壳统一使用共享 helper 绘制
* 减少 `game.py` 中重复的 `pygame.Surface` 和 `pygame.draw.rect` 面板外壳代码
* 删除旧的 `draw_transparent_rounded_rect()` 兼容 helper
* 整理 `game.py` 顶部 `config` import
* 支持星泡萝卜完美品质基础判定
* 星泡萝卜收获时有 10% 概率获得完美品质
* 收获时会提示普通品质或完美品质
* 完美品质暂时不影响售价
* 图鉴会记录是否获得过完美品质
* 完美品质记录会保存到 JSON 存档
...
| 按 `C`            | 打开或关闭图鉴面板               |
| 按 `Esc`          | 关闭当前打开的面板               |
| 按 `1`            | 商店关闭时：预览清晨 Morning      |
| 按 `2`            | 商店关闭时：预览白日 Daytime      |
| 按 `3`            | 商店关闭时：预览傍晚 Evening      |
| 按 `4`            | 预览午夜 Midnight           |
| 按 `0`            | 恢复真实时间昼夜阶段              |
| 点击窗口关闭按钮         | 自动保存并退出游戏               |
...
| Version 1.6   | 正式商店面板基础版           | 已完成 |
| Version 1.7   | 任务栏 UI 基础版          | 已完成 |
| Version 1.8   | 中文提示统一整理版           | 已完成 |
| Version 1.9   | UI 结构整理与小型重构        | 已完成 |
| Version 2.0   | 完美品质系统基础版         | 已完成 |
...
下一步建议为 Version 2.1 — 知识锁图鉴基础版。

Version 2.1 目标：

* 获得完美品质后解锁对应作物的最佳种植方式
* 图鉴显示已解锁的知识描述
* 未解锁前继续显示“未解锁”
* 解锁状态写入 JSON 存档
* 暂时只先接入星泡萝卜
