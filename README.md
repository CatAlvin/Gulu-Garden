...
当前版本已经完成基础种菜闭环、简单任务系统、图鉴基础版、多作物基础版，并加入土地解锁基础系统：
...
* locked 土地会显示解锁价格
* 点击 locked 土地可以尝试解锁
* 金币足够时，点击 locked 土地会扣除金币并解锁土地
* 金币不足时，会提示当前金币数量和还差多少金币
* 解锁后的土地会变为空地，可以正常播种
* 土地解锁状态会保存到 JSON 存档
* 重启游戏后，已解锁土地仍然保持可用
* 旧存档中缺失 `unlock_cost` 字段时，会自动补齐解锁价格
* 异常存档中的土地解锁状态会被规范化
* 支持星泡萝卜、云糖南瓜、月露蘑菇三种作物
* 支持多作物种子购买、播种、成长、收获和图鉴记录
* 支持现实时间离线成长
* 支持昼夜背景切换与场景素材调色
* 支持简单任务系统和图鉴基础系统
